...
/* Communication */
#define devTraceDataPin   PD0
#define devTraceReadyPin  PD1
#define devTraceAckPin    PD2
#define devTraceComPORT   PORTD
#define devTraceComPIN    PIND
#define devTraceComDRR    DDRD
 ...
 */
  ...
  __do_clear_bss_loop:
    st   -X, r1
    cpi  r26, lo8(devRAMbegin) /* hi8(devRAMbegin) and hi8(devRAMbegin) are assumed to be zero! */
    brne __do_clear_bss_loop
    rjmp __do_device_init
  /* Catch the timer 1 interrupt here if needed. */
    reti
  __tick_interrupt_vector:
  /* Catch the timer interrupt here 0x000D. */
    rjmp privTickYield
  __do_device_init:
    ldi  r24, (1<<devCLKPCE)
  ...
  __do_copy_data:
    rjmp    main
  __wdt_interrupt_vector:
  /* Catch the watchdog interrupt here 0x0012. */
